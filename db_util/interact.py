...
def db_control_dim(meta_table, data_table, *var_attr, comm_key=TIME_ATTR,
    db_path=DB_PATH, db_name=DB_NAME, db_temp=ANALYSIS_DB, temp_table=ANALYSIS_TABLE):
    """
    ...
    l_attr_meta_flt = [item for item in l_attr_meta_flt if item not in [TIME_ATTR]]
    ...
    for flt in control_var:
        flt_cond = ['{}.{}={}'.format(meta_table, var_attr[i], flt[i]) for i in range(len(flt))]
        temp_table_i = temp_table.format(','.join(flt_cond))
